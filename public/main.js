...
let N = document.getElementById("number_of_cars").value;
...
const traffic = [
    ...
    new Car (road.getLaneCenter(0), -1570, 30, 50, "DUMMY", 2),
    new Car (road.getLaneCenter(2), -1670, 30, 50, "DUMMY", 2),
    new Car (road.getLaneCenter(0), -1970, 30, 50, "DUMMY", 2),
...
function animate(time) {
    ...
    carCtx.save();
    carCtx.translate(0, -bestCar.y + carCanvas.height*0.70);
    
    road.draw(carCtx);
    for (let i = 0; i < traffic.length; i++) {
        traffic[i].draw(carCtx, "rgba(255, 0, 0, .7)");
    }
